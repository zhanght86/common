 ...
 * sFilter 1.1
 * 
 * author: 少侠 update: 2010-09-27 index: http://www.enjoyphp.com
 * 
 * *********************demo********************** <html> <head> <script
 * type='text/javascript' src='jquery.js'></script> <script
 * type='text/javascript' src='sfilter.js'></script> <link rel="stylesheet"
 * href='sfilter.css' type="text/css" />
 * 
 * <script type="text/javascript"> $(document).ready(function(){
 * $("#test").sfilter(); }); </script> </head> <body> <select id="test"> <option
 * value="0">所有品牌</option> <option value='70' >A ABC</option> <option
 * value='191' >A 安泰</option> <option value='428' >A 安吉尔</option> <option
 * value='122' >A 安可</option> <option value='327' >A 爱宁</option> </select>
 * </body> </html>
 */
(function($) {
	$.fn.unfilter = function() {
		this.unbind("focus");
	},
	$.fn.sfilter = function() {
		...
		this.before("<div class='cls_sfilter' id='sfilter_div" + name_round + "'>" + "<input type='text' style='width:" + (s_width - 2) + "px;' id='sfilter_text" + name_round + "' class='cls_sfilter_text' />" + "<ul  class='cls_sfilter_list' id='sfilter_list" + name_round + "'></ul>" + "</div>");
		...
		// 列表li的事件绑定
		sfilter_list.mouseover(function() {
			mouse_on_list = 1;
			$("#show").val(mouse_on_list);
		}).mouseout(function() {
			mouse_on_list = 0;
		...
		function filter_in() {
			sfilter_list.addClass("cls_sfilter_list").empty();
			...
			for (i = 1; i <= _select_size; i++) {
				if ((sfilter_text_value.length != 0) && (_select_text[i].toUpperCase().indexOf(sfilter_text_value.toUpperCase()) == -1)) {
					...
					sfilter_list.append("<li title='" + $('<div></div>').text(_select_value[i]).html() + "'\">" + $('<div></div>').text(_select_text[i]).html() + "</li>");
					// alert("<li title='"+_select_value[i]+"'
					// \">"+_select_text[i]+"</li>");
				}
			}

			sfilter_list.children("li").mouseover(function() {
				if (now_select_li != undefined) {
		...
		function to_select(svalue) {
			_select.val(svalue).trigger("change");
		}
	};
})(jQuery);