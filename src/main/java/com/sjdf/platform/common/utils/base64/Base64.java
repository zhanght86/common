 ...
 * informations.
 * </p>
 * Date: 2013-08-02 16:47
 *
 * @author ketqi
 */
public abstract class Base64 {

    public static final int ONE_LINE_LENGTH = 76;
    //the scope of char
    protected static final String SHARED_CHARS = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
    protected static final char SHARED_PAD = '=';
    ...
    public static byte[] encode(byte[] bytes, int wrapAt) {
        try (ByteArrayInputStream inputStream = new ByteArrayInputStream(bytes); ByteArrayOutputStream outputStream = new ByteArrayOutputStream()) {
    ...
    public static void encode(File source, File target, int wrapAt) throws IOException {
        try (InputStream inputStream = new FileInputStream(source); OutputStream outputStream = new FileOutputStream(target)) {
    ...
    private static void copy(InputStream inputStream, OutputStream outputStream) throws IOException {
        // 1KB buffer
        byte[] b = new byte[CommonPlatformConstant.LENGTH_1024];
        int len;
